...
// Extrait de StandardValueCalculatorScreen.kt : enum ESeries
...
// Extrait : fonction utilitaire pour formater les valeurs (k, M, G, m)
...
/**
 * Calcule la valeur standard la plus proche pour une valeur désirée et une série E.
 * Renvoie null si la valeur désirée est invalide (<= 0).
 */
fun computeNearestStandardValue(desired: Double, series: ESeries): Double? {
    if (desired <= 0) return null
    val seriesValues = series.values
    val magnitude = 10.0.pow(floor(log10(desired)))
    val normalizedDesired = desired / magnitude
    val nearestNormalized = seriesValues.minByOrNull { abs(it - normalizedDesired) } ?: return null
    return nearestNormalized * magnitude
}
